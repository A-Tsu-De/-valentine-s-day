const COUNT = 1; // <-- ПОМЕНЯЙ: сколько фоток лежит в папке images (1.jpg..N.jpg)
const photos = Array.from({ length: COUNT }, (_, i) => `images/${i + 1}.jpg`);
...
let taps = [];
const LIMIT = 7;       // сколько действий
const INTERVAL = 1700; // за сколько мс
...
function goPrev() {
  ...
  // если на финале и нажали назад — вернуться к последней фотке
...
frame.addEventListener("touchend", (e) => {
  ...
  if (Math.abs(dy) > Math.abs(dx)) return; // это скролл, не свайп
  if (dt > 700) return;
